...
def run_volcano(data, identifier, args={'alpha':0.05, 'fc':2, 'colorscale':'Blues', 'showscale': False, 'marker_size':8, 'x_title':'log2FC', 'y_title':'-log10(pvalue)', 'num_annotations':10}):
    ...
    for group in grouping.groups:
        ...
        if 'posthoc padj' in data.columns:
            pvalue_col = 'posthoc pvalue'
            padj_col = 'posthoc padj'
        else:
            pvalue_col = 'pvalue'
            padj_col = 'padj'

        signature = signature.sort_values(by=padj_col,ascending=True)
        
        pvals = []
        for index, row in signature.iterrows():
            # Text
            text.append('<b>'+str(row['identifier'])+": "+str(index)+'<br>Comparison: '+str(row['group1'])+' vs '+str(row['group2'])+'<br>log2FC = '+str(round(row['log2FC'], ndigits=2))+'<br>p = '+'{:.2e}'.format(row[pvalue_col])+'<br>FDR = '+'{:.2e}'.format(row[padj_col]))
